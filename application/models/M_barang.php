...
class M_barang extends CI_Model 
{
	...
	function cari_kode($keyword, $registered)
	{
		...
		if(count($koma) == 1)
		{
			$not_in .= " AND `kode_ikan` != '".$registered."' ";
		}

		$sql = "
			SELECT 
				`kode_ikan`, `nama_ikan_pembelian`, `harga` 
			FROM 
				`pj_nama_ikan_pembelian` 
			WHERE 
				`status` = '1' 
				
				AND ( 
					`kode_ikan` LIKE '%".$this->db->escape_like_str($keyword)."%' 
					OR `nama_ikan_pembelian` LIKE '%".$this->db->escape_like_str($keyword)."%' 
				) 
				".$not_in." 
		";
