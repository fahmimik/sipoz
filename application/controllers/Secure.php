			...
			if($this->form_validation->run() == TRUE)
			{
				...
				if($validasi_login->num_rows() > 0)
				{
					...
					$session = array(
						...
						'ap_kecamatan' => $data_user->id_kecamatan,
						'ap_kelurahan' => $data_user->id_kelurahan,
						'ap_posyandu' => $data_user->id_posyandu,
						'ap_nama_posyandu' => $data_user->nama_posyandu,
						'ap_level_caption' => $data_user->level_caption 
					);
					$this->session->set_userdata($session);	

					$URL_home = site_url('welcome');
