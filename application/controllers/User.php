		...
		
		{
			if($_POST)
			{
				...
				if($this->form_validation->run() == TRUE)
				{
					...
					$id_kelurahan 	= $this->input->post('lurah');
					$id_akses	= $this->input->post('id_akses');
					$status		= $this->input->post('status');

					$insert = $this->m_user->tambah_baru($username, $password, $nama,$id_kecamatan,$id_kelurahan, $id_akses, $status);
