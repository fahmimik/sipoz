...
class Penjualan extends MY_Controller 
{
	...
	public function transaksi()
	{
		...
		if($level == 'admin' OR $level == 'Kader' OR $level == 'Bidan' OR $level == 'kasir')
		{
			if($_POST)
			{
				if( ! empty($_POST['kode_ukuran_ikan']))
				{
					$total = 0;
					foreach($_POST['kode_ukuran_ikan'] as $k)
					{
						if( ! empty($k)){ $total++; }
					}
					...
					if($total > 0)
					{
						...
						$no = 0;
						foreach($_POST['kode_ukuran_ikan'] as $d)
						{
							if( ! empty($d))
							{
								$this->form_validation->set_rules('kode_ukuran_ikan['.$no.']','Kode ukuran_ikan #'.($no + 1), 'trim|required|max_length[40]');
							...
							else
							{
								...
								if($master)
								{
									...
									foreach($_POST['kode_ukuran_ikan'] as $k)
									{
										if( ! empty($k))
										{
											$kode_ukuran_ikan 		= $_POST['kode_ukuran_ikan'][$no_array];
											$jumlah_beli 			= $_POST['jumlah_beli'][$no_array];
											$harga_satuan 			= $_POST['harga_satuan'][$no_array];
											$sub_total 				= $_POST['sub_total'][$no_array];
											$id_master_ukuran_ikan	= $this->m_master_ikan->get_id_ukuran_ikan($kode_ukuran_ikan)->row()->id_master_ukuran_ikan;
											
											$insert_detail	= $this->m_penjualan_detail->insert_detail($id_master, $id_master_ukuran_ikan, $jumlah_beli, $harga_satuan, $sub_total);
											if($insert_detail)
											{
												$this->m_master_ikan->update_stok($id_master_ukuran_ikan, $jumlah_beli);
												$inserted++;
											}
										}
	...
	public function ajax_pelanggan()
	{
		if($this->input->is_ajax_request())
		{
			...
			$json['suplier_notelp']			= ( ! empty($data->suplier_notelp)) ? $data->suplier_notelp : "<small><i>Tidak ada</i></small>";
			$json['suplier_alamat']			= ( ! empty($data->suplier_alamat)) ? preg_replace("/\r\n|\r|\n/",'<br />', $data->suplier_alamat) : "<small><i>Tidak ada</i></small>";
			$json['info_lain']	= ( ! empty($data->info_lain)) ? preg_replace("/\r\n|\r|\n/",'<br />', $data->info_lain) : "<small><i>Tidak ada</i></small>";
	...
	public function ajax_kode()
	{
		if($this->input->is_ajax_request())
		{
			...
			$barang = $this->m_master_ikan->cari_kode($keyword, $registered);
			...
			if($barang->num_rows() > 0)
			{
				...
				foreach($barang->result() as $b)
				{
					$json['datanya'] .= "
						<li>
							<b>Kode</b> : 
							<span id='kodenya'>".$b->kode_ukuran_ikan."</span> <br />
							<span id='barangnya'>".$b->ukuran_ikan."</span>
	...
	public function pelanggan_json()
	{
		...
		{ 
			...
			$nestedData[]	= $row['nomor'];
			$nestedData[]	= $row['suplier_nama'];
			$nestedData[]	= preg_replace("/\r\n|\r|\n/",'<br />', $row['suplier_alamat']);
			$nestedData[]	= $row['suplier_notelp'];
			$nestedData[]	= $row['waktu_input'];
			
			if($level == 'admin' OR $level == 'Kader' OR $level == 'Bidan' OR $level == 'kasir' OR $level == 'keuangan') 
			{
				$nestedData[]	= "<a href='".site_url('penjualan/pelanggan-edit/'.$row['id_suplier'])."' id='EditPelanggan'><i class='fa fa-pencil'></i> Edit</a>";
			}

			if($level == 'admin' OR $level == 'Kader' OR $level == 'Bidan') 
			{
				$nestedData[]	= "<a href='".site_url('penjualan/pelanggan-hapus/'.$row['id_suplier'])."' id='HapusPelanggan'><i class='fa fa-trash-o'></i> Hapus</a>";
			}
