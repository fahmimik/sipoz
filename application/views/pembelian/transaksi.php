			...
			<div class='row'>
				<div class='col-sm-3'>
					...
					<div class="panel panel-primary" id='PelangganArea'>
						<div class="panel-heading"><i class='fa fa-user'></i> Pilih Pemasok/Nelayan</div>
						<div class="panel-body">
							<div class="form-group">
								<label>Pelanggan</label>
								<a href="<?php echo site_url('pembelian/tambah-pelanggan'); ?>" class='pull-right' id='TambahPelanggan'>Tambah Baru ?</a>
								<select name='id_pelanggan' id='id_pelanggan' class='form-control input-sm' style='cursor: pointer;'>
									...
									if($pelanggan->num_rows() > 0)
									{
										...
										{
											echo "<option value='".$p->id_suplier."'>".$p->suplier_nama."</option>";
										}
									}
									?>
								</select>
				...
				<div class='col-sm-9'>
					<h2>Transaksi Pembelian Ikan</h2>
					
					<table class='table table-bordered' id='TabelTransaksi'>
						<thead>
							<tr>
								<th style='width:35px;'>#</th>
								<th style='width:210px;'>Kode Ikan</th>
								<th>Nama Ikan</th>
					...
					<div class='row'>
						...
						<div class='col-sm-5'>
							<div class="form-horizontal">
								...
								<div class="form-group">
								    <label class="col-sm-6 control-label">Klik jika Lunas </label>
									<div class="col-sm-6">
									    	<input type='checkbox' name='lunas' id='lunas' class='form-control' onkeypress='return check_int(event)'>
										<input type='hidden' id='UangKembali' class='form-control' disabled>
									</div>
								</div>
								<div class='row'>
...
$(document).ready(function(){
	...
	$('#id_pelanggan').change(function(){
		...
		{
			$.ajax({
				url: "<?php echo site_url('pembelian/ajax-pelanggan'); ?>",
				type: "POST",
				cache: false,
...
function BarisBaru()
{
	...
	var Baris = "<tr>";
		Baris += "<td>"+Nomor+"</td>";
		Baris += "<td>";
			Baris += "<input type='text' class='form-control' name='kode_ikan[]' id='pencarian_kode' placeholder='Ketik Kode / Nama Barang'>";
			Baris += "<div id='hasil_pencarian'></div>";
		Baris += "</td>";
		Baris += "<td></td>";
		Baris += "<td>";
			Baris += "<input type='text' class='form-control' id='harga_satuan' name='harga_satuan[]' disabled>";
			Baris += "<span></span>";
		Baris += "</td>";
...
function AutoCompleteGue(Lebar, KataKunci, Indexnya)
{
	...
	$.ajax({
		url: "<?php echo site_url('pembelian/ajax-kode'); ?>",
		type: "POST",
		cache: false,
...
$(document).on('click', '#daftar-autocomplete li', function(){
	...
	$('#TabelTransaksi tbody tr:eq('+Indexnya+') td:nth-child(4) input').removeAttr('disabled').val(Harganya);

	$('#TabelTransaksi tbody tr:eq('+Indexnya+') td:nth-child(5) input').removeAttr('disabled').val(0);
...
function SimpanTransaksi()
{
	...
	FormData += "&cash="+$('#TotalBayarHidden').val();
	FormData += "&catatan="+encodeURI($('#catatan').val());
	FormData += "&grand_total="+$('#TotalBayarHidden').val();
	
	if($('#lunas').is(":checked"))
	{
	     FormData += "&status_lunas=1"; 
	   
	}
	else
	{
	    FormData += "&status_lunas=0";
	...
	$.ajax({
		...
		success: function(data){
			...
			{
				alert(data.pesan);
				window.location.href="<?php echo site_url('pembelian/transaksi'); ?>";
			}
			else 
			{
				$('.modal-dialog').removeClass('modal-lg');
