...
<div class="container-fluid">
	<div class="panel panel-default">
		<div class="panel-body">
			<h5 class=" col-lg-8 "><i class='fa fa-users fa-fw'></i> DAFTAR PASANGAN USIA SUBUR (PUS)</h5>
			<div><a href="<?php echo site_url('pus/cetak_exl')?>" class='btn btn-success pull-right' id='Cetak'><i class='fa fa-file-excel-o'></i> Cetak</a></div>
			<hr />
				...
				<table id="my-grid" class="table table-striped table-bordered">
					<thead>
						<tr>
							...
							<th rowspan=2 style='text-align:center'>Nama Ibu</th>
							<th rowspan=2 style='text-align:center'>Tempat, tgl Lahir</th>
							<th rowspan=2 style='text-align:center'>Nama Suami</th>
							<th rowspan=2 style='text-align:center'>Kontrasepsi</th>
							<th rowspan=2 style='text-align:center'>No HP</th>
							<th colspan=2 style='text-align:center'>Jumlah Anak</th>
							
							<th rowspan=2 class='no-sort' style='text-align:center'>Edit</th>
							<th rowspan=2 class='no-sort' style='text-align:center'>Hapus</th>
						</tr>
						<tr>
							<th>Hidup</th>
							<th>Wafat</th>
...
<?php
$tambahan = nbs(2)."<a href='".site_url('pus/tambah')."' class='btn btn-default' id='TambahUser'><i class='fa fa-plus fa-fw'></i> Tambah Data</a>";
...
<script type="text/javascript" language="javascript" >
	$(document).ready(function() {
		var dataTable = $('#my-grid').DataTable( {
			...
			"ajax":{
				url :"<?php echo site_url('pus/pus-json'); ?>",
				type: "post",
				error: function(){ 
					$(".my-grid-error").html("");
	...
	$(document).on('click', '#EditUsers', function(e){
		e.preventDefault();
		if($(this).attr('id') == 'EditUsers')
		{
			$('.modal-dialog').removeClass('modal-lg');
			$('.modal-dialog').removeClass('modal-sm');
			$('#ModalHeader').html('Riwayat Kontrasepsi');
		}
		$('#ModalContent').load($(this).attr('href'));
		$('#ModalGue').modal('show');
	});
