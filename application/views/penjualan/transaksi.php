	...
	<div class="panel panel-default">
		<div class="panel-body">
			...
			<div class='row'>
				...
				<div class='col-sm-9'>
				<h2>Transaksi Penjualan Ikan</h2>
					<table class='table table-bordered' id='TabelTransaksi'>
						<thead>
							<tr>
								...
								<th style='width:105px;'>Jumlah [Mc]</th>
		...
		if($(this).val() !== '')
		{
			$.ajax({
				...
				success: function(json){
					$('#telp_pelanggan').html(json.suplier_notelp);
					$('#alamat_pelanggan').html(json.suplier_alamat);
					$('#info_tambahan_pelanggan').html(json.info_lain);
				}
			});
		}
		else
...
function BarisBaru()
{
	...
	var Baris = "<tr>";
		Baris += "<td>"+Nomor+"</td>";
		Baris += "<td>";
			Baris += "<input type='text' class='form-control' name='kode_ukuran_ikan[]' id='pencarian_kode' placeholder='Ketik Kode / Nama Barang'>";
			Baris += "<div id='hasil_pencarian'></div>";
		Baris += "</td>";
		Baris += "<td></td>";
		Baris += "<td>";
			Baris += "<input type='txt' class='form-control' id='harga_satuan' name='harga_satuan[]' disabled>";
			Baris += "<span></span>";
		Baris += "</td>";
...
$(document).on('keyup', '#pencarian_kode', function(e){
	if($(this).val() !== '')
	{
		...
		else if(charCode == 13)
		{
			...
			$('#TabelTransaksi tbody tr:eq('+$(this).parent().parent().index()+') td:nth-child(4) input').removeAttr('disabled').val(to_rupiah(Harganya));
...
$(document).on('keyup', '#jumlah_beli', function(){
	...
	$.ajax({
		...
		type: "POST",
		cache: false,
		data: "kode_ukuran_ikan="+encodeURI(KodeBarang)+"&stok="+JumlahBeli,
		dataType:'json',
